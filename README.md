...
Hecho en Python + Tkinter. Reconstruido desde cero a partir del bytecode de la app
original *Seven.FM 2.0* y reemplazando la dependencia de Spotify por reproducción
**100% local**.
...
- **Letras**:
  - `.lrc` al lado del audio → letras **sincronizadas**.
  - `.txt` al lado → letra plana.
...
- **Análisis de mastering** (`numpy` + `soundfile`, en segundo plano):
  ...
  - **True Peak** (sobremuestreo 4×) + **detección de clipping**.
...
```
dist\UTMOST.FM.exe        ← versión estándar
dist\UTMOST.FM_PRO.exe    ← versión PRO
```

> ℹ️ Al ser una app indie sin firma digital de pago, Windows SmartScreen puede mostrar
> una advertencia azul la primera vez. Click en **"Más información" → "Ejecutar de todas
> formas"**.

1. Abre el `.exe`.
2. Click en el **engranaje** (arriba a la derecha).
3. **EXAMINAR** → elige tu carpeta de música → ajusta tema/idioma/etc. → **GUARDAR Y APLICAR**.
...
```
player.py            reproductor estándar
...
Reconstruido y ampliado a partir de *Seven.FM 2.0*.
